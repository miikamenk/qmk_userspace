...
static void format_uptime(char *buf, size_t bufsize) {
    ...
    snprintf(buf, bufsize, "%u:%02u:%02u", h, m, s);
}
...
#    endif
    ...
    oled_write_P(oled_get_layer_name(), false);
    ...
    if (mods & MOD_MASK_SHIFT) {
        if (idx < sizeof(modbuf) - 1) modbuf[idx++] = 'S';
    }
    if (mods & MOD_MASK_CTRL) {
        if (idx < sizeof(modbuf) - 1) modbuf[idx++] = 'C';
    }
    if (mods & MOD_MASK_ALT) {
        if (idx < sizeof(modbuf) - 1) modbuf[idx++] = 'A';
    }
    if (mods & MOD_MASK_GUI) {
        if (idx < sizeof(modbuf) - 1) modbuf[idx++] = 'G';
    }
    if (idx == 0) {
        modbuf[0] = '-';
        modbuf[1] = '\0';
    ...
    oled_write(modbuf, false);

    // Locks
    oled_set_cursor(0, 5);
    host_keyboard_led_state_t led = host_keyboard_led_state();
    oled_write_P(PSTR("Caps:"), false);
    oled_write(led.caps_lock ? PSTR("ON ") : PSTR("OFF"), false);
    oled_write_P(PSTR(" Num:"), false);
    oled_write(led.num_lock ? PSTR("ON ") : PSTR("OFF"), false);
    oled_write_P(PSTR(" Scr:"), false);
    oled_write(led.scroll_lock ? PSTR("ON") : PSTR("OFF"), false);

    // WPM
    oled_set_cursor(0, 7);
    char wpm_buf[16];
    snprintf(wpm_buf, sizeof(wpm_buf), "WPM: %u", get_current_wpm());
    ...
    oled_write(uptime_buf, false);

    // Footer
    oled_set_cursor(0, 13);
    oled_write_P(PSTR("qmk · oled 128x64"), false);
